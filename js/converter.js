class Converter {
    ...
    openDatabase() {
        if (!('indexedDB' in window)) {
            console.log('This browser doesnot support indexedDB');
    ...
    getCurrencyRateFromCache(fromCurrency, toCurrency) {
       ...
       return this.dbPromise.then(db => {
           ...
           return store.index('query').get(query);
             ...
             console.log('No rate found in the cache, Sorry:', error);
             return null;
        });
    }
        ...
        return this.dbPromise.then( db => {
            ...
            return index.getAll().then( currencies => {
                ...
                for (let currency of currencies){
                    document.getElementById("fromCurrencyList").innerHTML += 
                        "<option value=\""+currency.id+"\">"+
                        currency.currencyName+" ("+
                        currency.currencySymbol+") "
                        "</option>";

                    document.getElementById("toCurrencyList").innerHTML += 
                        "<option value=\""+currency.id+"\">"+
                        currency.currencyName+" ("+
                        currency.currencySymbol+") "
                        "</option>";
